...
\requirementsTable{
    RF1.1 & L’utente deve poter visualizzare i dataset proposti   & UC1 \tabularnewline
    RF1.2 & L’utente deve poter visualizzare i dettagli dataset selezionato   & UC1.1\par UC1.1.1\par UC1.1.2\par UC1.1.3\par \tabularnewline
    RF1.3 & L’utente deve poter caricare il dataset selezionato   & UC1.2 \tabularnewline

    RF1.4 & L’utente deve poter visualizzare i dati del dataset selezionato in forma tabellare   & UC2 \tabularnewline
    
    RF1.5 & L’utente deve poter visualizzare i dati del dataset selezionato in forma di grafico tridimensionale a barre verticali   & UC3 \tabularnewline
    
    RF1.6 & L’utente deve poter visualizzare gli assi del grafico tridimensionale    & UC4 \tabularnewline
    RF1.7 & L’utente deve poter visualizzare l`asse globale X del grafico tridimensionale    & UC4.1 \tabularnewline
    RF1.8 & L’utente deve poter visualizzare l`asse globale Y del grafico tridimensionale    & UC4.2 \tabularnewline
    RF1.9 & L’utente deve poter visualizzare l`asse globale Z del grafico tridimensionale    & UC4.3 \tabularnewline
    
    RF1.10 & L’utente deve poter visualizzare la legenda del grafico tridimensionale    & UC5 \tabularnewline
 
    RF1.11 & L’utente deve poter effettuare un`azione di pan nell` ambiente, spostando la telecamera sugli assi locali Y e X   & UC6 \tabularnewline
    
    RF1.12 & L’utente deve poter ruotare la telecamera a suo piacimento per visualizzare meglio il grafico tridimensionale   & UC7 \tabularnewline
    RF1.13 & L’utente deve poter ruotare la telecamera attorno all`asse globale X   & UC7.1 \tabularnewline
    RF1.14 & L’utente deve poter ruotare la telecamera attorno all`asse globale Y   & UC7.2 \tabularnewline
    
    RF1.15 & L’utente deve poter muovere la telecamera all`interno dell` ambiente   & UC8 \tabularnewline
    RF1.16 & L’utente deve poter muovere la telecamera sull`asse globale X   & UC8.1 \tabularnewline
    RF1.17 & L’utente deve poter muovere la telecamera sull`asse globale Y   & UC8.2 \tabularnewline
    RF1.18 & L’utente deve poter muovere la telecamera sull`asse globale Z   & UC8.3 \tabularnewline
    
    RF1.19 & L’utente deve poter riposizionare la telecamera alla sua posizione iniziale   & UC9 \tabularnewline
...
\requirementsTable{
    RQ1.1 & Il software deve essere sviluppato seguendo le metriche e il modello di qualità descritti nel documento \textit{Norme di Progetto}   & Decisione interna \tabularnewline
    RQ1.2 & Il software deve essere sviluppato pubblicando il codice sorgente sul repository Github dedicato   & Decisione interna \tabularnewline
    RQ1.3 & Il software deve essere sviluppato in modo tale da supportare grandi volumi di dati   & Capitolato \tabularnewline
    RQ1.4 & L'architettura del software deve permettere con agilità di poter aggiungere nuove funzionalità e possibilità di iterazione con il grafico    & Capitolato \tabularnewline
    RQ1.5 & La suite di test deve essere robusta per garantire ciò specificato in RQ4   & Capitolato \tabularnewline
    RQ2.6 & Il software deve essere testato attraverso test di tipo e2e   & Capitolato \tabularnewline
    % CHIEDERE AL PROPONENTE  
}
        
\subsection{Requisiti di vincolo}
\requirementsTable{
    RV1.1 & Il software deve essere sviluppato utilizzando Typescript come linguaggio di programmazione primario   & Decisione interna \tabularnewline
    RV1.2 & Il software deve essere sviluppato utilizzando la libreria React per la creazione di un interfaccia visiva sempice   & Decisione interna \tabularnewline
    RV1.3 & Il software deve essere sviluppato utilizzando la libreria Three.js   & Decisione interna \tabularnewline
    
    % PLACEHOLDER -> CHIEDERE AL PROPONENTE
    RV1.4 & Il software deve essere compatibile dalla versione 110 del browser Chrome    & Decisione interna \tabularnewline
}
...
\SrcToReqTable{
    ...
    Decisione interna & RQ1.1\par RQ1.2\par RV1.1\par RV1.2\par RV1.3\tabularnewline
...
\requirementsSummaryTable{
    Funzionale & 31 & 1 & 0 & 32 \tabularnewline
    Di qualità & 5 & 1 & 0 & 6 \tabularnewline
    Di vincolo & 3 & 0 & 0 & 3 \tabularnewline
