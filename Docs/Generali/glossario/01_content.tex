...
\section{A}
    \subsection{Action}
    (s. c. f.)\\
    Combinazione di script e configurazioni che permettono di eseguire operazioni 
...
\section{I}
    ...
    priorità e il flusso di lavoro di supervisione.
\pagebreak
\section{J}
\pagebreak
\section{K}
...
\section{R}
    \subsection{React}
    (s. p. m.)\\
    Libreria JavaScript per costruire interfacce utente basata su componenti, che utilizza un 
    approccio dichiarativo e un Virtual DOM per ottimizzare le prestazioni.
    \subsection{React Three Drei}
    \label{React Three Drei}
    (s. p. f.)\\
    Libreria che fornisce un insieme di componenti e helper per 
    la costruzione di applicazioni 3D utilizzando React e Three.js.
