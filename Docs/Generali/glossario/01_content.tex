...
\section{P}
    ...
    \label{PB}
    (s. p. f.)\\
    Acronimo di "Product Baseline", indica la seconda e ultima revisione di avanzamento del progetto effettuata dai docenti (committenti), durante la quale si verifica gli obiettivi funzionali e qualitativi del prodotto. Nel mezzo di questa fase, il prodotto viene presentato al proponente per l'accettazione come Minimum Viable Product (MVP).
    \subsection{PoC}
    (s. p. m.)\\
...
\section{R}
    ...
    \label{RTB}
    (s. c. f.)\\
    Acronimo di "Requirements and Technology Baseline", indica la prima revisione di avanzamento del progetto effettuata dai docenti (committenti) per esaminare i requisiti fissati con il proponente. Viene valutato le tecnologie e i framework scelti tramite un \nameref{Proof of Concept} (PoC). L'esito può richiedere modifiche e correzioni.
\pagebreak
\section{S}
    \subsection{Scrum}
    (s. p. m.)\\
